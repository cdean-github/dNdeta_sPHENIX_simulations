...
void ApplysPHENIXBeamParameter(PHHepMCGenHelper *HepMCGen, const Input::BeamConfiguration &beam_config)
{
    ...
    case AA_COLLISION:
        // heavy ion mode
        HepMCGen->set_vertex_distribution_mean(-0.022, 0.223, -4.03, 0.); // Initial AuAu 2024
        ...
        HepMCGen->set_vertex_distribution_width(120e-4,                                // approximation from past STAR/Run16 AuAu data
                                                120e-4,                                // approximation from past STAR/Run16 AuAu data
                                                9.7,                                   // sPH-TRG-2022-001. Fig B.2
    ...
    case pA_COLLISION:
        ...
        // 1.5mRad is split among both beams, means set to 0.75 mRad
        HepMCGen->set_beam_direction_theta_phi(localbcross, 0, M_PI - localbcross, 0); // 1.5mrad x-ing of sPHENIX
        HepMCGen->set_vertex_distribution_width(100e-4,                                // set to be similar to AA
    ...
    case pp_COLLISION:
        ...
        // 1.5mRad is split among both beams, means set to 0.75 mRad
        HepMCGen->set_beam_direction_theta_phi(localbcross, 0, M_PI - localbcross, 0); // 1.5mrad x-ing of sPHENIX
        HepMCGen->set_vertex_distribution_width(120e-4,                                // approximation from past PHENIX data
    ...
    HepMCGen->set_vertex_distribution_function(PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus);
}

//! apply sPHENIX nominal beam parameter according to the beam collision setting of Input::IS_PP_COLLISION
//! \param[in] HepMCGen any HepMC generator, e.g. Fun4AllHepMCInputManager, Fun4AllHepMCPileupInputManager, PHPythia8, PHPythia6, ReadEICFiles
void ApplysPHENIXBeamParameter(PHHepMCGenHelper *HepMCGen) { ApplysPHENIXBeamParameter(HepMCGen, Input::BEAM_CONFIGURATION); }
...
void ApplyEICBeamParameter(PHHepMCGenHelper *HepMCGen)
{
    ...
    HepMCGen->set_beam_direction_theta_phi(EIC_hadron_crossing_angle, // beamA_theta
                                           0,                         // beamA_phi
                                           M_PI,                      // beamB_theta
                                           0                          // beamB_phi
    );
    HepMCGen->set_beam_angular_divergence_hv(119e-6, 119e-6, // proton beam divergence horizontal & vertical, as in EIC CDR Table 1.1
    ...
    HepMCGen->set_beam_angular_z_coefficient_hv(-EIC_hadron_crossing_angle / 2. / z_hadron_cavity, 0, -EIC_hadron_crossing_angle / 2. / z_e_cavity, 0);
    ...
    HepMCGen->set_vertex_distribution_width(sigma_p_h * sigma_e_h / sqrt(sigma_p_h * sigma_p_h + sigma_e_h * sigma_e_h), // x
                                            sigma_p_v * sigma_e_v / sqrt(sigma_p_v * sigma_p_v + sigma_e_v * sigma_e_v), // y
                                            collision_sigma_z,                                                           // z
                                            collision_sigma_t);                                                          // t
    HepMCGen->set_vertex_distribution_function(PHHepMCGenHelper::Gaus,                                                   // x
